...
def RunModelDecisionTree(df_test2):
    ...
    temp = "Predicted Label using Wavelets Preprocessing & Decision Tree Classification is: " + str(prediction[0])
    return temp
...
if st.button('Run Model'):
   # If the user has uploaded a file
    if uploaded_file is not None:
        ...
        # Running the RunModelDecisionTree() function on all the different names in the dataframe
        for name, group in x_test.groupby("name"):
            st.write(str(name) + " - " + RunModelDecisionTree(group))
    else:
